...
class SkipThoughts:
  ...
  def _sequence(self, sentence, clean=False, eos_token=True):
  ...
  def _encoding(self, sentences):
    values = self._get_embeddings(sentences)
    a = tf.stack(values,0)
    return a
#    inputs = tf.placeholder(tf.int32, shape=shape, name="decoder_inputs")
#    return sess.run(self._get_thought, feed_dict={self._inputs: sentence})
#    return [self._thought(sent) for sent in sentences]
#    return sess.run(self._get_embeddings, feed_dict={inputs: sentences})


  def decode(self, thought):
    """Internally used to run the model agnostic to input feeding method."""
    decoder_in = tf.stack([thought for _ in range(self._batch_size)],0)
    if self._cuda:
      decoder_out = tf.contrib.cudnn_rnn.CudnnGRU(
        1, self._output_size, direction='unidirectional')(decoder_in)[0]
      ...
      rnn_cell = tf.contrib.cudnn_rnn.CudnnCompatibleGRUCell(self._output_size)
      sos_tokens = tf.constant([2]*self._batch_size, dtype=tf.int32)
#      print tf.reduce_sum(tf.sign(sos_tokens), reduction_indices=int(not self._time_major))

      helper = tf.contrib.seq2seq.GreedyEmbeddingHelper(
         embedding=self._get_embeddings,
         start_tokens=sos_tokens,
         end_token=tf.constant(3, dtype=tf.int32))
      decoder = seq2seq.BasicDecoder(rnn_cell, helper, decoder_in)
      decoder_out = seq2seq.dynamic_decode(
        decoder, output_time_major=self._time_major)[0].rnn_output
    words = []
    return tf.get_default_session().run(decoder_out)
